...
import openpyxl
import json
from pubchem_3D.items import Pubchem3DItem
...
for r in range(10):   #find_excel_rows()-1
    ...
    def parse(self, response):
        ...
        for query in query_list:
            ...
            try:
                ...
                yield scrapy.Request(url=url,callback=self.parse_second,meta={'cas':cas,'index':index,'query':query})
            except:
                # 把未下载的cas号传递存储给pending_cas_list
                pending_cas_list.append(query)
                print('Warning:--第{0}条数据的cas号可能有问题...正在记录'.format(index))
    ...
    def parse_second(self,response):
        try:
            ...
            down_src = 'https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/CID/' + str(
                ...
                obj_cid)
            pic=Pubchem3DItem(down_src=down_src,obj_cid=obj_cid,cas=cas,index=index,query=query)
            print(pending_cas_list)
            yield pic
        except:
            pending_cas_list.append(query)
